...
class Product
{
  ...
  {
    $sql = "SELECT p.*, c.nombre AS 'categoria' FROM productos p INNER JOIN categorias c ON p.categoria_id = c.id WHERE p.categoria_id = {$this->category_id} AND p.stock>0 ORDER BY id ASC";
    $products = $this->database->query($sql);
    return $products;
  }
